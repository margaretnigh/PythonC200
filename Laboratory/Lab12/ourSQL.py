...
def createTable(cursor, tableName, columns):
    """
    Given the cursor, the table, and the string with the columns, create the table 
in the database. 
    This will also handle REMOVING the table if it already exists. 
    columns example:
    str = "c1 TEXT, c2 INT, c3 INT"
    """
    theQuery = "CREATE TABLE {tableName} ({columns})"
...
def insertValues(cursor, tableName, valueString):
    ...
    """
    theQuery = f"INSERT INTO {tableName} VALUES ({valueString})"
    cursor.execute(theQuery)
...
def queryStatement(theSelect, theFrom, theWhere=""):
    ...
    """
    theQuery = f"SELECT {theSelect} FROM {theFrom}"
    if theWhere != "":
        theQuery += f" WHERE {theWhere}"
    return theQuery
